...
interface NotificationComingSoonProps {
  className?: string
  title?: string
  description?: string
...
const NotificationComingSoon: React.FC<NotificationComingSoonProps> = ({
  className = '',
  ...
  return (
    <section className={`w-full ${className}`}>
      <div className='mb-5'>
        <h1 className='text-[24px] font-semibold tracking-[-0.02em] text-slate-900'>
          Thông báo của tôi
        </h1>
      </div>

      <div className='overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-[0_8px_30px_rgba(15,23,42,0.04)]'>
        <div className='flex min-h-[82px] items-center gap-2 overflow-x-auto border-b border-slate-100 px-4 sm:px-5'>
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`relative flex h-[82px] shrink-0 items-center gap-2.5 px-4 text-sm font-medium transition-colors ${
              ...
              >
                ...
                {isActive && (
                  <span className='absolute inset-x-2 bottom-0 h-[2px] rounded-full bg-blue-600' />
                )}
              </button>
            )
          })}
